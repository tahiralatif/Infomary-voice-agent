...
async def _save_lead(
    ...
    if sheet_result["success"] and email_result["success"]:
        return f"Lead saved. Lead ID: {lead['lead_id']}"
    return "Partial save — check logs."
...
save_lead = StructuredTool.from_function(
    ...
    name="save_lead",
    description="Save the senior care lead to Google Sheet and send email notification to the team.",
